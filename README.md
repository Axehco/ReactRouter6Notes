...
注意：router6里面要求，想实现自定义类名，要把类写成一个函数的形式。
...
const res = useMatch('/home/messages/detail/:id/:title/:content')  // 另一种方式

search:  useSearchParams
<NavLink to={`detail?id=${v.id}&title=${v.title}&content=${v.content}`}>{v.title}</NavLink>
{ path: 'detail', element: <Detail /> }
const [search, setSearch] = useSearchParams(); const id = search.get('id')
...
App组件和连同他左右的子组件都是处于路由的上下文环境中的。 为**假**的时候脱离了路由器的管理。一般你是用第三方的组件，就可以去判断。
...
// 输出
{pathname: '/user', search: '?id=0018&name=19', hash: '#qwe'}
```
